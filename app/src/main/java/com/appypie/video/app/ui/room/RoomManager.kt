...
import com.appypie.video.app.webservices.AuthServiceException
import com.twilio.androidenv.Env
import com.twilio.video.*
import timber.log.Timber

class RoomManager(private val context: Context, private val sharedPreferences: SharedPreferences) {
    ...
    fun connectToRoom(identity: String, roomName: String, isNetworkQualityEnabled: Boolean) {
        ...
        try {
            ...
            val token = AppPrefs.getString(Constants.TWILIO_TOKEN)
    ...
    private fun setSdkEnvironment(sharedPreferences: SharedPreferences) {
        val env = sharedPreferences.getString(Preferences.ENVIRONMENT, Preferences.ENVIRONMENT_DEFAULT)
        val nativeEnvironmentVariableValue = EnvUtil.getNativeEnvironmentVariableValue(env)
        Env.set(context, EnvUtil.TWILIO_ENV_KEY, nativeEnvironmentVariableValue, true)
    }
